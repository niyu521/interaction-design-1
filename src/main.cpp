...
//    It is recommended that as many surfaces as possible be oriented to the
//    north.
//
...
static constexpr const float coefficient_tbl[3] = {0.5f, (1.0f / 256.0f),
                                                   (1.0f / 1024.0f)};
...
void updateCalibration(uint32_t c, bool clear = false) {
  ...
  if (clear) {
    ...
    if (c) { // Start calibration.
      ...
      // ※ The actual calibration operation is performed each time during
      // M5.Imu.update.
      //
      // There are three arguments, which can be specified in the order of
      // Accelerometer, gyro, and geomagnetic. If you want to calibrate only the
      // Accelerometer, do the following. M5.Imu.setCalibration(100, 0, 0);
      //
    ...
    } else { // Stop calibration. (Continue calibration only for the geomagnetic
             // sensor)
...
void loop(void) {
  ...
  if (imu_update) {
    ...
    [6~8]=mag
        ...
        M5_LOGV("ax:%f  ay:%f  az:%f", data.accel.x, data.accel.y,
    data.accel.z); M5_LOGV("gx:%f  gy:%f  gz:%f", data.gyro.x , data.gyro.y ,
    data.gyro.z ); M5_LOGV("mx:%f  my:%f  mz:%f", data.mag.x  , data.mag.y  ,
    data.mag.z  );
    //*/
    ++frame_count;
  } else {
